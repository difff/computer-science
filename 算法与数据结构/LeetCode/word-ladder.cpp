...
public:
    int ladderLength(string start, string end, unordered_set<string> &dict) {
  		unordered_map< string, vector<string> > childsCache;
  		unordered_set<string> pushedStrings;
		queue<string> queue;
		vector<int> levelNodeCounts;
		int pushingLevelCount;
		int poppingLevel;
		int popCount;

		dict.insert(start);
		queue.push(end);
		pushedStrings.insert(end);		
...
//			getchar();
			...
			vector<string> childStrings = getChildStrings(pushedStrings, childsCache, dict, queue.front());
			for (vector<string>::iterator iter = childStrings.begin(); iter != childStrings.end(); ++iter) {
				queue.push(*iter);
				pushedStrings.insert(*iter);
				pushingLevelCount++;
//				cout << "new child: " << *iter << endl;
				if (start.compare(*iter) == 0)
					return poppingLevel+2;
			}
//			cout << "pop father: " << queue.front() << endl;
			queue.pop();
			popCount++;
			if (popCount == levelNodeCounts[poppingLevel]) {
				levelNodeCounts.push_back(pushingLevelCount);
				popCount = pushingLevelCount = 0;
//				cout << "finshed the " << poppingLevel << "th level" << endl;
				poppingLevel++;
			}
		}
		return 0;
	...
	vector<string> getChildStrings(unordered_set<string> &pushedStrings, unordered_map< string, vector<string> > &childsCache,
		unordered_set<string> &dict, string farStr) 
    {
    	if (childsCache.find(farStr) != childsCache.end())
    		return childsCache[farStr];
    	vector<string> childStrs;
    	for (int i = 0; i != farStr.length(); ++i) {
    		string childString = farStr;
    		for (int j = 0; j != 26; ++j) {
    			childString[i] = 'a' + j;
    			if ( dict.find(childString) != dict.end() && pushedStrings.find(childString) == dict.end() && childString.compare(farStr) != 0 )
    				childStrs.push_back(childString);
    		}
    	}
    	childsCache[farStr] = childStrs;
    	return childStrs;
    }
};
...
public:
    int ladderLength(string start, string end, unordered_set<string> &dict) {
		...
		pushingLevelCount = 0;
		while (!queue.empty()) {
			vector<string> childStrings = getChildStrings(childsCache, dict, queue.front());
			for (vector<string>::iterator iter = childStrings.begin(); iter != childStrings.end(); ++iter) {
				queue.push(*iter);
				pushingLevelCount++;
				dict.erase(*iter);
	...
	vector<string> getChildStrings(unordered_map< string, vector<string> > &childsCache,
		unordered_set<string> &dict, string farStr) 
    {
    	if (childsCache.find(farStr) != childsCache.end())
    		return childsCache[farStr];
    	...
    	for (int i = 0; i != farStr.length(); ++i) {
    		...
    		for (int j = 0; j != 26; ++j) {
    			childString[i] = 'a' + j;
    			if ( dict.find(childString) != dict.end() && childString.compare(farStr) != 0 )
    				childStrs.push_back(childString);
    		}
    	}
    	childsCache[farStr] = childStrs;
...
public:
    int ladderLength(string start, string end, unordered_set<string> &dict) {
		...
		vector<string> (*getChilds)(unordered_map< string, vector<string> > &childsCache,
		unordered_set<string> &dict, string farStr);
		getChilds = dict.size() > 26 ? getChildStrings1 : getChildStrings2;

		while (!queue.empty()) {
			vector<string> childStrings = getChilds(childsCache, dict, queue.front());
			for (vector<string>::iterator iter = childStrings.begin(); iter != childStrings.end(); ++iter) {
				queue.push(*iter);
				pushingLevelCount++;
				dict.erase(*iter);
	...
	static vector<string> getChildStrings1(unordered_map< string, vector<string> > &childsCache,
		unordered_set<string> &dict, string farStr) 
    {
    	if (childsCache.find(farStr) != childsCache.end())
    		return childsCache[farStr];
    ...
    static vector<string> getChildStrings2(unordered_map< string, vector<string> > &childsCache,
		unordered_set<string> &dict, string farStr)
    {
 	    if (childsCache.find(farStr) != childsCache.end())
    		return childsCache[farStr];
    	...
    	for (unordered_set<string>::iterator i = dict.begin(); i != dict.end(); ++i)
    		if (canTransform(farStr, *i))
    			childStrs.push_back(*i);
    	childsCache[farStr] = childStrs;
    	return childStrs;
    }

    static inline bool canTransform(string curStr, string nextStr) 
    {
    	int count = 0;
...
public:
    int ladderLength(string start, string end, unordered_set<string> &dict) {
		...
		dict.erase(end); // 可要可不要
		queue.push(levelEnd);
		childsCache[levelEnd] = vector<string>(1, levelEnd);
		while (true) {
			vector<string> childStrings = getChildStrings(queue.front(), dict, childsCache);
			for (vector<string>::iterator child = childStrings.begin(); child != childStrings.end(); ++child) {
				if (child->compare(start) == 0)
					return poppingLevel+1;
				queue.push(*child);
				dict.erase(*child);
			}
			if (queue.front().compare(levelEnd) == 0) { 
				queue.pop();
				poppingLevel++;
				if (queue.front().compare(levelEnd) == 0)
					break;
	...
	vector<string> getChildStrings(string farStr, unordered_set<string> &dict,
								   unordered_map< string, vector<string> > &childsCache) 
    {
    	if (childsCache.find(farStr) != childsCache.end())
    		return childsCache[farStr];
    	...
    	for (int i = 0; i != farStr.length(); ++i) {
    		...
    		for (int j = 0; j != 26; ++j) {
    			childString[i] = 'a' + j;
    			if ( dict.find(childString) != dict.end() )
    				childStrs.push_back(childString);
    		}
    	}
    	childsCache[farStr] = childStrs;
    		...
    		if (farStr != "") {
    			for (int i = 0; i != farStr.length(); ++i) {
		    		...
		    		for (char ch = 'a'; ch <= 'z'; ++ch) {
    					childString[i] = ch;
    					if (childString == start)
    						return poppingLevel + 1;
	    				if ( dict.count(childString) > 0 ) {
    						queue.push(childString);
	    					dict.erase(childString);
	    				}
		    		}
		    	}
    		} else {
...
class SolutionTwoBFS {
public:
    int ladderLength(string start, string end, unordered_set<string> &dict) 
    {
		queue<string> queueA, queueB;
		unordered_set<string> setA, setB;
		unordered_map<string, int> stringLevel;
		...
		while (!queueA.empty() || !queueB.empty()) {
			string farStrA = queueA.front();
			queueA.pop();
   			for (int i = 0; i != farStrA.length(); ++i) {
   				string childString = farStrA;
	    		for (char ch = 'a'; ch <= 'z'; ++ch) {
   					childString[i] = ch;
   					if ( setB.count(childString) > 0)
   						return stringLevel[childString] + stringLevel[farStrA];
    				if ( dict.count(childString) > 0 ) {
   						queueA.push(childString);
   						setA.insert(childString);
    					dict.erase(childString);
    					stringLevel[childString] = stringLevel[farStrA] + 1;
    				}
	    		}
	    	}
    		string farStrB = queueB.front();
			queueB.pop();
   			for (int i = 0; i != farStrB.length(); ++i) {
   				string childString = farStrB;
	    		for (char ch = 'a'; ch <= 'z'; ++ch) {
   					childString[i] = ch;
   					if ( setA.count(childString) > 0)
   						return stringLevel[childString] + stringLevel[farStrB];
    				if ( dict.count(childString) > 0 ) {
   						queueB.push(childString);
   						setB.insert(childString);
    					dict.erase(childString);
    					stringLevel[childString] = stringLevel[farStrB] + 1;
    				}
	    		}
	    	}
	    }
...
/*
所有push过的都不能再选
poplingLevel来记录当前poping的层次
增加一个farStr有两个好处：
1是不会写太多queue.front()
2是此时你可以获得队列的头两个元素

关于C++和STL
在微弱的效率提升和可读性方面，我选择可读性
比如string的[]和==都是内部重载了，属于语法糖，但是可读性很强
再比如dict.find(childString) != dict.end()表示找到了
其实可读性并不好，一个!=给人感觉是找不到
而dict.count(childString)意思是set中childString（或map中以childString为value的pair）有多少个
显然dict.count(childString)的可读性比dict.find(childString) != dict.end()好
另外push("")会有临时对象的产生，相对增加一点点开销，代码直观可读更重要
*/
...
int main(int argc, char const *argv[])
{
 ...
 //    "fr","nb","yb","if","pb","ge","th","pm","rb","sh","co","ga","li","ha","hz",
	...
	// string start = "hot";
	// string end = "dog";
	// unordered_set<string> myset({"hot","dog","dot"});
	...
	// string start = "hot";
	// string end = "dog";
	// unordered_set<string> myset({ "hot", "dog"});
	...
	// unordered_set<string> myset({"hot", "dot", "dog", "lot", "log", "dof", "mit", 
	// "sit","set", "mog", "mig", "seg", "nax", "max"});

	// unordered_set<string> myset({ "hot", "dot", "dog", "lot", "log", "cog" });
//	unordered_set<string> myset({ "hot", "dot", "dog", "lot", "log", "cog", "hit"});

	SolutionTwoBFS solution;
	cout << solution.ladderLength(start, end, myset) << endl;
	return 0;
}
