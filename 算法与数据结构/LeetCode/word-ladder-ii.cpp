...
public:
	struct Node {
		Node *father;
		string str;
		Node(Node *f, string s):father(f), str(s) {}
		void print() {
			cout << "father="  << father << ", "  << "str=" << str << endl;
		}
	};

    vector< vector<string> > findLadders(string start, string end, unordered_set<string> &dict) 
    {
    	return getAllRoad(buildTree(start, end, dict));
	}

private:
	unordered_map< string, vector<string> > childsCache;

	vector< vector<string> > getAllRoad(const vector<Node *> &leafNodes)
	{
		vector< vector<string> > roadVector;	
		for (vector<Node *>::const_iterator iter = leafNodes.begin(); iter != leafNodes.end(); ++iter) {
			vector<string> road;
			Node *p = *iter;
			while (p->father != NULL) {
				road.push_back(p->str);
				p = p->father;
			}
			road.push_back(p->str);
	...
	vector<Node *> buildTree(string start, string end, unordered_set<string> &dict) 
	{
		dict.insert(start);

		vector<Node *> leafNodes;
//		unordered_set<string> topStrings;
		queue<Node *> queue;
		Node *pRootNode = new Node(NULL, end);
		queue.push(pRootNode);

		//topStrings.insert(end);
		dict.erase(end);

		vector<int> levelNodeCountVector;
		levelNodeCountVector.push_back(1);
		
		vector<string> currentLevelStringVector;
		int currentLevel = 0;
		int popCount = 0;
		...
		while (!queue.empty()) {
			...
			getchar();
#endif
			vector<string> childStrings = getChildStrings(dict, queue.front()->str);
			for (vector<string>::iterator iter = childStrings.begin(); iter != childStrings.end(); ++iter) {
				Node *pNode = new Node(queue.front(), *iter);
				queue.push(pNode);
				currentLevelStringVector.push_back(*iter);
				if (start.compare(*iter) == 0)
					leafNodes.push_back(pNode);
#ifdef DEBUG_X_Y_Z
				cout << "new child: " << pNode->str << endl;
#endif			
			}
#ifdef DEBUG_X_Y_Z
			cout << "pop father: " << queue.front()->str << endl;
#endif
			queue.pop();
			popCount++;
			if (popCount == levelNodeCountVector[currentLevel]) {
				if (leafNodes.size() != 0)
					break;
				levelNodeCountVector.push_back(currentLevelStringVector.size());
#ifdef DEBUG_X_Y_Z
				cout << "finshed the " << currentLevel << "th level" << endl;
				cout << "get those string in this level into topStrings:" << endl;
#endif
				for (vector<string>::iterator iter = currentLevelStringVector.begin(); iter != currentLevelStringVector.end(); ++iter)
					dict.erase(*iter); // 同一级的string有可能重复，所以这里删除有成功有失败
					//topStrings.insert(*iter);
				currentLevelStringVector.clear();
				popCount = 0;
				currentLevel++;
			}
		}
    ...
    vector<string> getChildStrings(unordered_set<string> &dict, string farStr) 
    {
    	// 不加cache 从400ms 边城1600ms
    	...
    	for (int i = 0; i != farStr.length(); ++i) {
    		...
    		for (int j = 0; j != 26; ++j) {
    			childString[i] = 'a' + j;
    			// 容易成功的放在前面 这个很关键
    			//  && childString.compare(farStr) != 0 可以不要 因为farStr一定不在dict中
...
//	unordered_set<string> myset({ "hot", "dot", "dog", "lot", "log", "cog", "hit"});
	...
	Solution solution;
	vector< vector<string> > ans = solution.findLadders(start, end, myset);

	VectorCategory vc;
	int cnt = 1;
	for (vector< vector<string> >::iterator iter = ans.begin(); iter != ans.end(); ++iter) {
//		cout << cnt++ << ":" <<  endl;
		vc.printVector(*iter);
	}
